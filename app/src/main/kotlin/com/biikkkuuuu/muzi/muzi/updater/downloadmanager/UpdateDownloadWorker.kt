package com.biikkkuuuu.muzi.muzi.updater.downloadmanager
...
class UpdateDownloadWorker(private val context: Context, workerParams: WorkerParameters) :
    ...
    override suspend fun doWork(): Result = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            val downloadFile = if (isZip) File(downloadDir, "echo_temp.zip") else File(downloadDir, "muzi.apk")
            ...
            val finalFile = if (isZip) {
                val targetApkFile = File(downloadDir, "muzi.apk")
                var extracted = false
                try {
